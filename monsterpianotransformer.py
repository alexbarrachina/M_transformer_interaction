...
def generate(model,
    ...
    # Check if model is a list and convert to proper model if needed
    if isinstance(model, list):
        model = model[0]  # Assuming the first element is the model
    ...
    with torch.amp.autocast(device_type=device, dtype=torch.bfloat16):
        ...
        x = torch.LongTensor([inputs] * 1).to(device) # (B, primer_len)
    ...
    y = out.tolist() # (B, num_gen_tokens)

    outputs = []

    if return_prime:
        for o in y: # for each batch, add the prime to the generated sequence
...
def generate_forced_dtime(model,
             input_tokens,
             num_gen_tokens=600,
             num_batches=1,
             temperature=1.0,
