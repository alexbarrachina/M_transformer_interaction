...
class Decoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        #self.token_emb = nn.Embedding(num_tokens, emb_dim) # Embedding function
...
#        nn.init.kaiming_normal_(self.token_emb.emb.weight)
    ...
    def forward(
       ...
       # Embed past notes, sum all embedding values, 
        ...
        past_embeds = ( # (1,4,768)
            self.dtime_emb(past_tokens['dtime']) + # past_notes['dtime'] (1,1024)
            self.pitch_emb(past_tokens['pitch']) +
            self.dur_emb(past_tokens['dur']) +
            self.button_emb(past_tokens['button'])
        )  # [B, T-1, emb_dim] (dtime_emb + pitch_emb + dur_emb + but_emb) -> note embeddings

       # Embed current tokens
        current_embeds = ( 
            self.dtime_emb(current_dtime) +
            self.button_emb(current_button)
        )  # [B, 1, emb_dim] (dtime_emb + but_emb) -> current note embedding
        
        # Concatenate past and current
        x = torch.cat([past_embeds, current_embeds], dim=1)  # [B, T, emb_dim]
    
        # embedding dropout
        x = self.emb_dropout(x)

        # ¿? Create a causal mask or supply your custom mask if needed
        # If you want to ensure the model can't see "future tokens", you can build a
        # [T, T] causal mask. But for single-step generation, you might not need a typical
        # AR mask, or you might have a partial mask for "past" vs "current" tokens.
        ...
        #  Only take the last position (the "current" token) for next-pitch prediction
        # x_last = x[:, -1]  # [B, d_model]
        # Project to pitch logits => [B, vocab_size_pitch]
        # logits = self.to_logits(x_last).unsqueeze(1)  # [B, 1, vocab_size_pitch]
...
class AutoregressiveAutoencoder(Module):
    ...
    def forward(self, note_tokens: Dict[str, Tensor]):
        ...
        e = self.encoder(note_tokens) # encoder output (batch, seq_len) (2, 1024, 1)
        b = self.quantizer(e) # generate buttons (batch, seq_len) (2, 1024, 1)
        ...
        # Create past tokens dictionary (excluding the last position)
        past_tokens = {
        ...
        '''past_tokens = {
            ...
            'dtime': torch.cat([torch.full_like(note_tokens['dtime'][:, :1], SOS), 
                           note_tokens['dtime'][:, :-1]], dim=1),
            'pitch': torch.cat([torch.full_like(note_tokens['pitch'][:, :1], SOS), 
                           note_tokens['pitch'][:, :-1]], dim=1),
            'dur': torch.cat([torch.full_like(note_tokens['dur'][:, :1], SOS), 
                         note_tokens['dur'][:, :-1]], dim=1),
            'button': torch.cat([torch.full_like(b[:, :1], SOS_BUTTONS), 
                           b[:, :-1]], dim=1)
        }'''
        logits = self.decoder(past_tokens, current_dtime, current_button) # (B, T (seq_len), VOCAB_SIZE_PITCH) (2, 1024, 128)
        # Only take the last position's output for prediction
        logits = logits[:, -1:]  # [B, 1, dim]

        # Replace ignore_index with pad_value
        # Replace ignore_index with pad_value
        #inp = torch.where(inp == ignore_index, self.pad_value, inp)

        # Target should be the pitch at the current (last) position
        target = note_tokens['pitch'][:, -1].unsqueeze(1)  # [B, 1]
        ...
        loss_contour = torch.square(
            torch.maximum(
                1 - torch.diff(note_tokens['pitch'], dim=1).float() * torch.diff(e, dim=1),
        ...
        notes_held = (note_tokens['pitch'][:, 1:] == note_tokens['pitch'][:, :-1]).float()
