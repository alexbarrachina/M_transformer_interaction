...
def train(cur_epoch, model, dataloader, loss, opt, lr_scheduler=None, num_iters=-1, save_checkpoint_steps=1000, tensorboard_steps=200):
    best_eval_acc        = 0.0
    ...
    with tqdm(total=len(dataloader)) as bar_train:
        for batch_num, batch in enumerate(dataloader):
            ...
            if save_steps % save_checkpoint_steps == 0:
                ...
                #plt.savefig('./SaveModel/gpt2_rpr_checkpoint_training_loss_graph.png')
...
def eval_model(model, dataloader, loss, num_iters=-1):
    ...
    with torch.set_grad_enabled(False):
        ...
        with tqdm(total=len(dataloader)) as bar_eval:
            ...
            steps += 1
            if(USE_TENSORBOARD):                
                tensorboard_summary.add_scalar("eval_loss", avg_loss, steps)
                tensorboard_summary.add_scalar("eval_acc", avg_acc, steps)  
...
def TrainNewModel(train_loader, val_loader, test_loader):
    ...
    best_eval_loss       = float("inf")
    best_eval_loss_epoch = -1
    best_acc_file = './SaveModel/gpt2_rpr_acc.pth'
    best_loss_file = './SaveModel/gpt2_rpr_loss.pth'
