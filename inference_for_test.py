...
model = load_model(model_name='full w new loss', device='cpu')
model.to(device)
...
b = model.gen_buttons(context).squeeze(0)
#b = model.quantizer.real_to_discrete(e)

for i in range(0, TOTAL_GEN_LEN-1-CTX_LEN):

  context = {
    'dtime': torch.tensor(dict_input_tokens['dtime'][i+1:i+1+CTX_LEN], dtype=torch.long).unsqueeze(0),
    'pitch': torch.tensor(dict_input_tokens['pitch'][i:i+CTX_LEN], dtype=torch.long).unsqueeze(0),
    'dur': torch.tensor(dict_input_tokens['dur'][i:i+CTX_LEN], dtype=torch.long).unsqueeze(0),
    'button': torch.tensor(b[i+1:i+1+CTX_LEN], dtype=torch.long).unsqueeze(0)
  }
