...
# Pre-encode all 3 style prompts to avoid latency on style switch
...
for _spath in style_prompt_midi_paths:
    ...
    _pitch_list = _style_tokens['pitch'][:style_seq_len]
    _valid_len = len(_pitch_list)
    if _valid_len < style_seq_len:
        _pitch_list = _pitch_list + [PAD_IDX] * (style_seq_len - _valid_len)
    _mask_list = [True] * _valid_len + [False] * (style_seq_len - _valid_len)
...
"""# KEYBOARD LISTENER — keys 1/2/3 switch active style on-the-fly """
