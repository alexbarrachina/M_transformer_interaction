...
sample_midi_path5 = './samples/billEvans.midi'
...
# While space is held, incoming MIDI notes are recorded here as a style sequence
# (they are NOT mapped to buttons and NOT fed to the model for inference).
space_held: bool = False
recorded_style_pitches: List[int] = []
...
def manageNote(note, velocity): 
  ...
  global space_held
  global recorded_style_pitches
  
  if TRACES:
    print("key", note)

  # Recording mode: while space is held, store the played MIDI pitches as a style
  # sequence and just monitor them audibly. Do not map to buttons, do not run inference.
  if space_held:
    if velocity > 0: # noteOn
        recorded_style_pitches.append(note)
        playNote(note, velocity)
        noteOn_dict[note] = (note, 0.0, -1)
    else: # noteOff
        if note in noteOn_dict:
            pitch, _, _ = noteOn_dict[note]
            playNote(pitch, 0)
...
def _on_key_press(key: pkeyboard.Key) -> None:
    ...
    if key == pkeyboard.Key.space:
        # on_press fires repeatedly on auto-repeat; only start a fresh recording
        # on the first press (transition from not-held to held).
        if not space_held:
            space_held = True
            recorded_style_pitches = []
            print("Recording style sequence... (release space to stop)")
        return
    try:
        ...
        if c == MOTIF_STYLE_KEY:
            if len(recorded_style_pitches) > 0:
                _sctx, _smask = _encode_style_pitch_list(recorded_style_pitches, repeat_to_style_len=True)
                style_contexts[MOTIF_STYLE_IDX] = _sctx
                style_context_masks_list[MOTIF_STYLE_IDX] = _smask
                ...
                motif_style_ready = True
                kv_cache = None
                print(f"Recorded style sequence active ({len(recorded_style_pitches)} notes)")
            else:
                print("No style sequence recorded yet. Hold space and play notes first.")
        if c in ('r'):
            print("resetting context")
            reset_context()
        if c in ('s'):
...
def _on_key_release(key: pkeyboard.Key) -> None:
    ...
    if key == pkeyboard.Key.space:
        space_held = False
        print(f"Style sequence recorded: {len(recorded_style_pitches)} notes. Press {MOTIF_STYLE_KEY} to activate.")
