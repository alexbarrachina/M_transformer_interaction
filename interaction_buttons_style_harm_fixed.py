#===================================================================================================
# Monster Genie interaction_buttons_style_harm_fixed.py Python module
# Interaction, generating buttons from MIDI keyboard,
# starting with a context extracted from a MIDI file
#
# FIXED-CHORD FiLM TEST: copy of interaction_buttons_style_harm.py where the
# ChordPlanner is BYPASSED. Pressing 'z'/'c'/'v'/'b' (STABILIZE / PREPARE /
# TENSION / RESOLVE) conditions the FiLM with a HAND-DERIVED chord for the
# active style piece (keys 1-5), looked up in FIXED_CHORDS below. This isolates
# the harmony conditioner: if generation doesn't follow these known-plausible
# chords, the FiLM path (not the planner) is the problem.
# The other movement keys (x/n/m/,/.) are disabled in this tester.
#
...
# =====================================================================================
# FIXED-CHORD TEST TABLE
# Hand-derived plausible chords per (style piece, movement), from docs/pieces/*.txt
# plus the MEASURED key of each sample MIDI (estimate_key_from_pitches on the file):
#   1 Bach BWV 846 prelude ........ C  major   (doc: I / ii42 / V65 / I loop, V7 at mm.3,18)
#   2 Clair de lune ............... Db major   (doc: I6 opening, ii7 m.12 + mm.15-18 wash, V7/V9 mm.13,19)
#   3 Chopin Nocturne Op.9 No.1 ... Bb MINOR   (the FILE is No.1 in Bb minor — the docs
#                                               describe No.2 in Eb major, so these come
#                                               from the measured key: i / iv / V7 / i)
#   4 Lotus Land .................. Eb minor   (measured; Bb drone, black-key pentatonic:
#                                               i7 field, iv7 soft predominant, V7 vs drone)
#   5 Gymnopédie No.1 ............. D  major   (doc: Imaj7/IVmaj7 rocking, ii7 mm.16/26-30,
#                                               v7 with C-natural m.15, plain-I cadence m.31)
# Movements follow the analyzer semantics: STABILIZE = assert/hold the center,
# PREPARE = predominant (build expectation), TENSION = dominant-type pull,
# RESOLVE = arrival on the (local) tonic.
# =====================================================================================
FIXED_TEST_MOVES = (MOVE_STABILIZE, MOVE_PREPARE, MOVE_TENSION, MOVE_RESOLVE)

# Keep the fixed tester on the checkpoint's trained harmony manifold by default.
# The current AE_style_harm_tester_v1 training pickle contains no KEY_CHANNEL
# events and all chord-label events have FUNC_UNKNOWN, so non-neutral
# function/key/mode embeddings are effectively untrained for this model.
FIXED_USE_EXPLICIT_ANALYSIS_FACTORS = False

# piece idx (0-4 == style keys 1-5) -> movement -> (root_pc, quality_id, function_id)
FIXED_CHORDS = {
    0: {  # Bach Prelude in C major
        MOVE_STABILIZE: (0,  QUALITY_MAJOR,            FUNC_TONIC),        # C   (I)
        MOVE_PREPARE:   (2,  QUALITY_MINOR_SEVENTH,    FUNC_SUBDOMINANT),  # Dm7 (ii7, m.2)
        MOVE_TENSION:   (7,  QUALITY_DOMINANT_SEVENTH, FUNC_DOMINANT),     # G7  (V7, mm.3/18)
        MOVE_RESOLVE:   (0,  QUALITY_MAJOR,            FUNC_TONIC),        # C   (I, mm.4/19)
    },
    1: {  # Debussy Clair de lune (Db major)
        MOVE_STABILIZE: (1,  QUALITY_MAJOR,            FUNC_TONIC),        # Db   (I, mm.1/3/5)
        MOVE_PREPARE:   (3,  QUALITY_MINOR_SEVENTH,    FUNC_SUBDOMINANT),  # Ebm7 (ii7, m.12 + mm.15-18)
        MOVE_TENSION:   (8,  QUALITY_DOMINANT_SEVENTH, FUNC_DOMINANT),     # Ab7  (V7, mm.13/19)
        MOVE_RESOLVE:   (1,  QUALITY_MAJOR,            FUNC_TONIC),        # Db   (I)
    },
    2: {  # Chopin Nocturne Op.9 No.1 (Bb minor — measured from the MIDI)
        MOVE_STABILIZE: (10, QUALITY_MINOR,            FUNC_TONIC),        # Bbm (i)
        MOVE_PREPARE:   (3,  QUALITY_MINOR,            FUNC_SUBDOMINANT),  # Ebm (iv, minor predominant)
        MOVE_TENSION:   (5,  QUALITY_DOMINANT_SEVENTH, FUNC_DOMINANT),     # F7  (V7)
        MOVE_RESOLVE:   (10, QUALITY_MINOR,            FUNC_TONIC),        # Bbm (i; the piece ends
                                                                           #  Picardy — try (10, QUALITY_MAJOR)
                                                                           #  to test that color)
    },
    3: {  # Cyril Scott Lotus Land (Eb minor — measured; Bb drone)
        MOVE_STABILIZE: (3,  QUALITY_MINOR_SEVENTH,    FUNC_TONIC),        # Ebm7 (i7 pentatonic field)
        MOVE_PREPARE:   (8,  QUALITY_MINOR_SEVENTH,    FUNC_SUBDOMINANT),  # Abm7 (iv7, softest predominant)
        MOVE_TENSION:   (10, QUALITY_DOMINANT_SEVENTH, FUNC_DOMINANT),     # Bb7  (V7 against the Bb drone)
        MOVE_RESOLVE:   (3,  QUALITY_MINOR,            FUNC_TONIC),        # Ebm  (soft plain-triad arrival)
    },
    4: {  # Satie Gymnopédie No.1 (D major)
        MOVE_STABILIZE: (2,  QUALITY_MAJOR_SEVENTH,    FUNC_TONIC),        # Dmaj7 (the piece's Imaj7)
        MOVE_PREPARE:   (4,  QUALITY_MINOR_SEVENTH,    FUNC_SUBDOMINANT),  # Em7   (ii7, mm.16/26-30)
        MOVE_TENSION:   (9,  QUALITY_MINOR_SEVENTH,    FUNC_DOMINANT),     # Am7   (v7 m.15 — modal
                                                                           #  C-natural pull; docs warn
                                                                           #  against a real V7 here)
        MOVE_RESOLVE:   (2,  QUALITY_MAJOR,            FUNC_TONIC),        # D     (plain I, m.31 cadence)
    },
}

# piece idx -> (key_pc, mode) written into the FiLM key factors
FIXED_PIECE_KEYS = {
    0: (0,  MODE_MAJOR),   # C  major
    1: (1,  MODE_MAJOR),   # Db major
    2: (10, MODE_MINOR),   # Bb minor
    3: (3,  MODE_MINOR),   # Eb minor
    4: (2,  MODE_MAJOR),   # D  major
}

FIXED_PIECE_NAMES = {
    0: 'Bach Prelude (C maj)',
    1: 'Clair de lune (Db maj)',
    2: 'Chopin Op.9/1 (Bb min)',
    3: 'Lotus Land (Eb min)',
    4: 'Gymnopedie 1 (D maj)',
}
...
last_midi_style_idx: int = active_style_idx  # last REAL midi style (0-4): the piece the fixed chords refer to
...
def _active_piece_idx() -> int:
    """Piece the fixed chords refer to: the active MIDI style (keys 1-5); when
    the captured-motif style (key 6) is active, the last real MIDI style."""
    if 0 <= active_style_idx < len(style_prompt_midi_paths):
        return active_style_idx
    return last_midi_style_idx


def _apply_fixed_chord(idx: int) -> None:
    """FIXED-CHORD TEST: bypass the ChordPlanner and condition the FiLM with the
    hand-derived chord for (active piece, requested movement) from FIXED_CHORDS.
    Same side effects as _plan_next_chord (factors + bass + chroma); `idx` kept
    for call-site parity."""
    global current_factors, current_chroma, current_bass_pc
    piece = _active_piece_idx()
    spec = FIXED_CHORDS[piece].get(int(current_movement))
    if spec is None:
        print(f"[fixed] no fixed chord for {MOVEMENT_NAMES.get(current_movement, current_movement)} "
              f"— only z/c/v/b are mapped in this tester")
        return
    root, qual, func = spec
    key_pc, mode = FIXED_PIECE_KEYS[piece]
    if not FIXED_USE_EXPLICIT_ANALYSIS_FACTORS:
        func = FUNC_UNKNOWN
        key_pc = PC_UNKNOWN
        mode = MODE_UNKNOWN
    current_factors = {'root_pc': int(root), 'quality_id': int(qual),
                       'function_id': int(func), 'key_pc': int(key_pc), 'mode': int(mode)}
    current_bass_pc = int(root)
    with torch.inference_mode():
        ch = model.chroma_from_factors(torch.tensor([int(root)]), torch.tensor([int(qual)]))
    current_chroma = ch.squeeze(0).cpu().tolist()
    print(f"[fixed] {FIXED_PIECE_NAMES[piece]} | {MOVEMENT_NAMES.get(current_movement, current_movement)}: "
          f"{_format_chord(current_factors, current_chroma)}")
...
def manageNote(note, velocity): 
  ...
  if velocity > 0: # noteOn
    ...
    # --- Harmony: on a movement request, apply the FIXED chord for the active
    # piece (planner bypassed); then record the harmony state for this position
    # so the decoder is FiLM-conditioned.
    idx = i + CTX_LEN
    if pending_plan:
        _apply_fixed_chord(idx)
        ...
        # Fixed tester: the chord never tracks the melody — re-applying just
        # rewrites the same fixed chord (kept so HARM_REPLAN_EVERY stays inert).
        _apply_fixed_chord(idx)
    _write_harm(idx)
...
def _on_key_press(key: pkeyboard.Key) -> None:
    ...
    try:
        ...
        if c in HARMONY_KEY_MAPPING:
            mv = HARMONY_KEY_MAPPING[c]
            if mv not in FIXED_TEST_MOVES:
                print(f"[fixed] {MOVEMENT_NAMES.get(mv, mv)} disabled in the fixed-chord "
                      f"tester — only z/c/v/b are mapped")
                return
            current_movement = mv
            pending_plan = True  # the next generated note applies the fixed chord
            piece = _active_piece_idx()
            root, qual, _func = FIXED_CHORDS[piece][mv]
            print(f"Movement: {MOVEMENT_NAMES.get(mv, mv)} -> fixed chord "
                  f"{_pc_name(root)} {_QUALITY_NAMES.get(qual, qual)} "
                  f"({FIXED_PIECE_NAMES[piece]})")
            return
        if c in ('1', '2', '3', '4', '5'):
            active_style_idx = int(c) - 1
            last_midi_style_idx = active_style_idx
            style_context = style_contexts[active_style_idx]
            style_context_mask = style_context_masks_list[active_style_idx]
            kv_cache = None
